...
public class PutawayServiceImpl implements PutawayService{
	...
	public PutawayServiceImpl(StorageBinRepository binRepository) {
		super();
	...
	public StorageBin findAvailableBin(Long warehouseId, Integer incomingQuantity, Product product) {
        ...
        for (StorageBin bin : allBins) {
            ...
            if (bin.getWarehouse() != null && bin.getWarehouse().getId().equals(warehouseId)) {
                
                // Rule 2: Match Zone/Category Restrictions
                if (bin.getAllowedCategory() != null && bin.getAllowedCategory().equalsIgnoreCase(productCategory)) {
                    
                    // Rule 3: Check Remaining Space capacity
                    int currentBinStock = 0;
                    if (bin.getInventoryItems() != null) {
                        currentBinStock = bin.getInventoryItems().stream()
                                             .mapToInt(item -> item.getQuantity())
                                             .sum();
                    }
                    
                    if (currentBinStock + incomingQuantity <= maxBinCapacity) {
                        return bin; // Safe allocation target found!
                    }
                }
            }
        }

        throw new RuntimeException("Putaway Error: No available bins found for category '" 
                + productCategory + "' with space for " + incomingQuantity + " units in warehouse ID: " + warehouseId);
	}
